...
bool argumentsHandler(StringVec args)
{
	...
	for(StringVec::iterator it = args.begin(); it != args.end(); ++it)
	{
		...
		if((*it) == "--version")
		{
			...
			"otPokemon Imperium - Pokemon Tibia Online.\n";
			return false;
		}
...
void allocationHandler()
{
	...
	g_game.saveGameState(true); // Thalles Vitor
	exit(-1);
}
...
ServiceManager* services)
{
	...
	std::cout << "A server developed by : GOD Vitor." << std::endl;
	std::cout << "otPokemon Imperium - Pokemon Tibia Online." << std::endl << std::endl;
			...
			switch(LOWORD(wParam))
			{
				...
				case ID_MENU_MAIN_CLEARLOG:
				{
					if(g_game.getGameState() != GAME_STATE_STARTUP)
					{
						...
						std::cout << "Compiled with " << BOOST_COMPILER << " at " << __DATE__ << ", " << __TIME__ << "." << std::endl;
						std::cout << "A server developed by: GOD Vitor." << std::endl;
						std::cout << "otPokemon Imperium - Pokemon Tibia Online ." << std::endl;
						std::cout << std::endl;
					}
				...
				{
					if(MessageBox(hwnd, "Voce tem certeza que quer desligar o servidor ?", "Shutdown", MB_YESNO) == IDYES)
					{
