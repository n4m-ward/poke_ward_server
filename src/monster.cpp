...
bool Monster::selectTarget(Creature* creature)
{
	...
	if (creature->getPlayer() && creature->summons.size() > 0)
		return false;
...
bool Monster::selectTarget2(Creature* creature)
{
#ifdef __DEBUG__
	std::cout << "Selecting target... " << std::endl;
#endif
	...
	if(!creature->isSummon() && creature->getMonster() && setAttackedCreature(creature))
		Dispatcher::getInstance().addTask(createTask(
			boost::bind(&Game::checkCreatureAttack, &g_game, getID())));
...
bool Monster::convinceGuardian(Creature* creature)
{
	Player* player = creature->getPlayer();
	if(player && !player->hasFlag(PlayerFlag_CanConvinceAll) && !mType->isConvinceable)
		return false;
