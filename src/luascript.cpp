...
Thing* ScriptEnviroment::getThingByUID(uint32_t uid)
{
	...
	if(uid >= 0x10000000)
	{
		Thing* tmp = g_game.getCreatureByID(uid);
		if(tmp)
		{
			if (!tmp->isRemoved()) {
				m_localMap[uid] = tmp;
...
Item* ScriptEnviroment::getItemByUID(uint32_t uid)
{
	...
	{
		if (tmp && !tmp->isRemoved()) { // Thalles Vitor
...
Container* ScriptEnviroment::getContainerByUID(uint32_t uid)
{
	...
	if(tmp && !tmp->isRemoved()) // Thalles Vitor
	{
		Container* container = tmp->getContainer();
		if(container) {
			return container;
		}
	}
...
ScriptEnviroment LuaScriptInterface::m_scriptEnv[51];
...
{
	//doPlayerSetStatus(cid, status)
	lua_register(m_luaState, "doPlayerSetStatus", LuaScriptInterface::luaDoPlayerSetStatus);

	//getPlayerStatus(cid)
	lua_register(m_luaState, "getPlayerStatus", LuaScriptInterface::luaDoPlayerGetStatus);
	...
	//doPushThing(cid, position)
	lua_register(m_luaState, "doPushThing", LuaScriptInterface::luaDoPushThing);
	...
	lua_register(m_luaState, "convertIPAddress", LuaScriptInterface::convertIPAddress);
	...
	//doCreatureSetNick2(cid, nick)
    lua_register(m_luaState, "doCreatureSetNick2", LuaScriptInterface::luaDoCreatureSetNick);
	...
	//doPlayerAddMoney2(cid, money)
	lua_register(m_luaState, "doPlayerAddMoney2", LuaScriptInterface::luaDoPlayerAddMoney);
	...
	//doConvinceGuardian(cid, target)
	lua_register(m_luaState, "doConvinceGuardian", LuaScriptInterface::luaDoConvinceGuardian);
	...
	//doMonsterSetTarget2(cid, target)
	lua_register(m_luaState, "doMonsterSetTarget2", LuaScriptInterface::luaDoMonsterSetTarget2);
	...
	//doPlayerSetLootRate(name, rate)
	lua_register(m_luaState, "setPlayerLootRate", LuaScriptInterface::luaDoPlayerSetLootRate);
	...
	//getGuardians(cid)
	lua_register(m_luaState, "getGuardians", LuaScriptInterface::luaGetCreatureGuardians);
	
	//dropPokemon(cid, pokeballId)
	lua_register(m_luaState, "dropPokemon", LuaScriptInterface::luaDoDropPokemon);

	//doUseItem(cid, item)
	lua_register(m_luaState, "doUseItem", LuaScriptInterface::luaDoUseItem);
    ...
    if (creature) {    
        ...
        for (SpectatorVec::const_iterator it = list.begin(); it != list.end(); ++it) {
           if (Player* player = (*it)->getPlayer()) {
               player->reloadCreatures(creature);
           }
        }
...
int32_t LuaScriptInterface::luaDoTeleportThing(lua_State* L)
{
	...
	ScriptEnviroment* env = getEnv();
	Thing* tmp = env->getThingByUID(popNumber(L));
	if (!tmp) {
		lua_pushboolean(L, false);
		return 1;
	}
...
int32_t LuaScriptInterface::luaDoCreateMonster(lua_State* L)
{
	//doCreateMonster(name, pos[, displayError = true])
	bool displayError = true;
	bool pokeHouse = false; // Thalles Vitor

	if (lua_gettop(L) > 3)
		pokeHouse = popNumber(L);
	...
	if(!g_game.placeCreature(monster, pos, false, pokeHouse))
	{
		delete monster;
		if(displayError)
...
int32_t LuaScriptInterface::luaDoPlayerAddMoney2(lua_State* L)
{
	//doPlayerAddMoney2(cid, money)
	uint64_t money = popNumber(L);
...
// Thalles Vitor
int32_t LuaScriptInterface::luaDoConvinceGuardian(lua_State* L)
{
	//doConvinceGuardian(cid, target)
	uint32_t cid = popNumber(L);
...
int32_t LuaScriptInterface::luaDoMonsterSetTarget2(lua_State* L)
{
	//doMonsterSetTarget2(cid, target)
	uint32_t targetId = popNumber(L);
	ScriptEnviroment* env = getEnv();
	...
	if (master) {
		...
		if (player) {
			player->sendCreatureSquare(target, (SquareColor_t)SQ_COLOR_RED);
		}
	} */
	return 1;
}
...
int32_t LuaScriptInterface::luaDoDropPokemon(lua_State* L)
{
    //dropPokemon(cid, pokeballId)
    ScriptEnviroment* env = getEnv();
    uint16_t pokeballId = popNumber(L);
    ...
    if(Player* player = env->getPlayerByUID(popNumber(L)))
	{
        lua_pushboolean(L, player->selectPokeball(pokeballId));
    }
    else
    {
        errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
...
int32_t LuaScriptInterface::luaDoUseItem(lua_State* L)
{
    //doUseItem(cid, item)
    ScriptEnviroment* env = getEnv();
	Item* item = env->getItemByUID(popNumber(L));
	...
	g_actions->useItem(player, item->getPosition(), INDEX_WHEREEVER, item);
    return 1;
}
...
int32_t LuaScriptInterface::luaDoPlayerSetLootRate(lua_State* L)
{
    //doPlayerSetLootRate(name, rate)
...
int32_t LuaScriptInterface::luaGetSpectators(lua_State* L)
{
	...
	uint64_t rangey = popNumber(L), rangex = popNumber(L);
...
int32_t LuaScriptInterface::luamoveCreatureTo(lua_State* L)
{
	...
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
	    ...
	    if(!g_game.getPathToEx(creature, pos, listDir, 1, 1, true, true))
	    {
         lua_pushboolean(L, false);                        
         return 1;
         }       
...
int32_t LuaScriptInterface::luaDoPushThing(lua_State* L)
{
	//doPushThing(cid, position)
	ScriptEnviroment* env = getEnv();
	uint32_t itemId = popNumber(L);
	...
	if(Player* player = env->getPlayerByUID(popNumber(L))) {
		pushThing(L, g_game.internalGetThing(player, position, INDEX_WHEREEVER, Item::items[itemId].clientId, STACKPOS_NORMAL));
		lua_pushboolean(L, true);
	}
...
int32_t LuaScriptInterface::luaDoPlayerSetStatus(lua_State* L)
{
	//doPlayerSetStatus(cid, status)
	std::string status = popString(L);
	...
	if(Player* player = env->getPlayerByUID(popNumber(L))) {
		...
		for (SpectatorVec::const_iterator it = list.begin(); it != list.end(); ++it) {
           if (Player* player2 = (*it)->getPlayer()) {
               player2->reloadCreatures(player);
           }
        }
...
int32_t LuaScriptInterface::luaDoPlayerGetStatus(lua_State* L)
{
	//getPlayerStatus(cid)
	ScriptEnviroment* env = getEnv();
	if(Player* player = env->getPlayerByUID(popNumber(L))) {
		lua_pushstring(L, player->getStatus().c_str());
	}

	lua_pushstring(L, "data/images/game/status/online.png");
	return 1;
}

// Thalles Vitor - Market System
int32_t LuaScriptInterface::luaDoSendMarketItem(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoRemovePlayerMarketItem(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoCancelPlayerMarketItem(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoOpenMarketSellerInsertMyOffers(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoOpenMarketBuyInsertAllOffers(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoMarketBuyOffer(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoMarketMyHistoric(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoMarketMakeOfferRemoveItem(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoMarketMakeOfferConfirm(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoOpenMarketViewYourOffers(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoOpenMarketInsertAllOffersInSlots(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoOpenMarketInsertAllOffersToMeInSlots(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoMarketCancelYourOfferBackBuyer(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoMarketViewOffersToMe(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoMarketOfferConfirmToMe(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoMarketReedemMyOffers(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoMarketChangeOption(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoMarketChangePage(lua_State* L)
{
	//sendMarketChangePage(cid, type, category)
...
int32_t LuaScriptInterface::luaDoMarketSearch(lua_State* L)
{
	//sendMarketSearch(cid, type, category)
	std::string category = popString(L);
	std::string type = popString(L);
...
int32_t LuaScriptInterface::convertIPAddress(lua_State* L)
{
	//convertIPAddress(ip)
	int ip = popNumber(L);
	char buffer[17];
