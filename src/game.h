...
class Game
{
	public:
		...
		// Thalles Vitor - Loot
		ReturnValue internalAddLoot(Cylinder* fromCylinder, Player* player, Item* item);
		...
		void saveGameStateAntiRollback(); // Thalles
		void loadGameState();
		...
		void setGlobalSaveMessage(int16_t key) {globalSaveMessage = key;}
		bool getGlobalSaveMessage(int16_t key) const {return globalSaveMessage;}
