...
bool IOMapSerialize::saveHouses()
{
	...
	for(HouseMap::iterator it = Houses::getInstance()->getHouseBegin(); it != Houses::getInstance()->getHouseEnd(); ++it)
		saveHouse(db, it->second); // Thalles
...
// Thalles Vitor
bool IOMapSerialize::saveHouse2(Database* db, House* house)
{
	DBQuery query;
	/* query << "UPDATE `houses` SET `owner` = " << house->getOwner() << ", `paid` = "
		<< house->getPaidUntil() << ", `warnings` = " << house->getRentWarnings() << ", `lastwarning` = "
...
bool IOMapSerialize::saveItemsHouse(Database* db, uint32_t tileId, uint32_t houseId, const Tile* tile)
{
	int32_t thingCount = tile->getThingCount();
	if(!thingCount)
		return true;
	...
	for(int32_t i = 0; i < thingCount; ++i)
	{
		...
		query2.str("");
		query2 << "DELETE FROM `tile_items` WHERE `tile_id` = " << houseTileId;
		...
		Position tilePosition = tile->getPosition();
		query << "INSERT INTO `tiles` (`id`, `world_id`, `house_id`, `x`, `y`, `z`) VALUES ("
		<< houseTileId << ", " << g_config.getNumber(ConfigManager::WORLD_ID) << ", " << houseId << ", "
		<< tilePosition.x << ", " << tilePosition.y << ", " << tilePosition.z << ")";
		if(!db->executeQuery(query.str()))
			return false;
		...
		query << houseTileId << ", " << g_config.getNumber(ConfigManager::WORLD_ID) << ", " << ++runningId << ", " << parentId << ", "
			<< item->getID() << ", " << (int32_t)item->getSubType() << ", " << db->escapeBlob(attributes, attributesSize);
		if(!query_insert.addRow(query.str()))
			return false;
