...
bool IOLoginData::getAccountId(const std::string& name, uint32_t& number)
{
	...
	// Thalles Vitor //
		std::list<std::string> allowedNames = {"[Help]", "[Tutor]", "[GM]", "[CM]", "[ADM]", "[CEO]", "[DEV]", "[YT]"};
		if (std::find(allowedNames.begin(), allowedNames.end(), name) != allowedNames.end()) {
...
bool IOLoginData::savePlayer(Player* player, bool preSave/* = true*/, bool shallow/* = false*/)
{
	...
	{
		uint32_t itemId = Item::items.getItemIdByName(db->escapeString((*it)).c_str());
		if (itemId > 0) {
...
bool IOLoginData::playerMail(Creature* actor, std::string name, uint32_t townId, Item* item)
{
	...
	// Thalles Vitor
	ReturnValue ret = g_game.internalMoveItem(NULL, item->getParent(), depot, INDEX_WHEREEVER,
		item, item->getItemCount(), NULL, FLAG_NOLIMIT);
	if(!depot || ret != RET_NOERROR)
	{
		if(player->isVirtual())
			delete player;

		ReturnValue ret2 = g_game.internalMoveItem(NULL, item->getParent(), player->getTile(), INDEX_WHEREEVER,
		item, item->getItemCount(), NULL, FLAG_NOLIMIT);
		if (ret2 != RET_NOERROR) {
			return true;
		}
...
bool IOLoginData::playerExists(std::string& name, bool multiworld /*= false*/, bool checkCache /*= true*/)
{
	...
	if (std::find(allowedNames.begin(), allowedNames.end(), name) != allowedNames.end()) {
		...
		std::replace_if(name.begin(), name.end(), ::isdigit, ' ');
		std::replace_if(name.begin(), name.end(), ::ispunct, ' ');
	}
	...
	query << "SELECT `id`, `name` FROM `players` WHERE `name` = " << db->escapeString(name) << " AND `deleted` = 0";
	if(!multiworld)
		query << " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID);
...
bool IOLoginData::playerExists2(std::string& name, bool multiworld /*= false*/, bool checkCache /*= true*/)
{
	if(checkCache)
	{
		GuidCacheMap::iterator it = guidCacheMap.find(name);
