...
// Thalles Vitor
void MonsterType::setLuckyRate(uint16_t rate)
{
   m_luckyrate = rate;
}

uint16_t MonsterType::getLuckyRate()
{
   return m_luckyrate;
}
...
bool Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading/* = false*/)
{
	...
	if(readXMLString(root, "race", strValue))
	{
		...
		else if(tmpStrValue == "fairy" || atoi(strValue.c_str()) == 21)
