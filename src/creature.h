...
#define EVENT_CREATURE_THINK_INTERVAL 500 // 500
...
class Creature : public AutoId, virtual public Thing
{
	...
	public:
		...
		void setGuardian(Creature* creature) { guardianMaster = creature;} // Thalles Vitor
		Creature* getMaster() {return master;}
		const Creature* getMaster() const {return master;}
		Creature* getGuardian() { return guardianMaster;} // Thalles Vitor
		const Creature* getGuardian() const { return guardianMaster;} // Thalles Vitor
		Player* getPlayerMaster() const {return isPlayerSummon() ? master->getPlayer() : NULL;}
		bool isSummon() const {return master != NULL;}
		bool isGuardian() const {return guardianMaster != NULL;} // Thalles Vitor
		...
		const std::list<Creature*>& getGuardians() {return guardians;} // Thalles Vitor
		void destroySummons();
		uint32_t getSummonCount() const {return summons.size();}

		// Thalles Vitor - Guardian
		void addGuardian(Creature* creature);
		void removeGuardian(const Creature* creature);
