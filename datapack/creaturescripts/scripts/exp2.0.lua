...
-- Thalles Vitor - Auto Potion
function onUseAutoPotion(cid)
	...
	local potions = {12343, 12345, 12346, 12347, 12348}
	local bp = getPlayerSlotItem(player, 3)
	if bp.uid > 0 then
		if isContainer(bp.uid) then
			for i = 0, getContainerSize(bp.uid)-1 do
				local item = getContainerItem(bp.uid, i)
				if item and item.uid > 0 and isInArray(potions, item.itemid) then
					doRemoveItem(item.uid, 1)

					local a = potions2[item.itemid]
					doHealOverTime(cid, a.div, 100, a.effect)
					break
				end
			end
		end
...
-- Thalles Vitor - Passive --
if isSummon(attacker) and isMonster(cid) then -- Seu summon atacando
	docastpassive(attacker)
end

if isMonster(attacker) and isSummon(cid) then -- Monstro atacando
	docastpassive(attacker)
end
--
...
-- Thalles Vitor - Buffs
	if isMonster(cid) and isMonster(attacker) then
		local type1 = pokes[getCreatureName(cid)].type
...
if isSummon(cid) then                                  
   ...
   if isCreature(master) and getPlayerStorageValue(master, 248759) == 1 then
      valor = valor * 1.1
   elseif isCreature(master) and getPlayerStorageValue(master, 248759) == 3 then
      valor = valor * 0.9
   end
end
if isSummon(attacker) then
...
if damageCombat == PSYCHICDAMAGE or damageCombat == MIRACLEDAMAGE then
   if isCreature(attacker) and getPlayerStorageValue(attacker, 95487) >= 1 then
