...
if(Modules == nil) then
	...
	function ShopModule:callbackOnBuy(cid, itemid, subType, amount, ignoreCap, inBackpacks)
		...
		if(a < amount) then
			...
			if a > 0 then
				if not doPlayerRemoveMoney(cid, ((a * shopItem.buy) + (b * 20))) then
					return false
				end
				return true
			end
			...
			msg = self.npcHandler:parseMessage(msg, parseInfo)
			if not doPlayerRemoveMoney(cid, totalCost) then
				return false
			end
	...
	function ShopModule:callbackOnSell(cid, itemid, subType, amount, ignoreCap, inBackpacks)
		...
		if(doPlayerRemoveItem(cid, itemid, amount, subType)) then
			...
			doPlayerAddMoney2(cid, amount * shopItem.sell)
			local playerMoney = getPlayerMoney(cid)
			doPlayerRemoveMoney(cid, playerMoney)
			doPlayerAddMoney2(cid, playerMoney)	
