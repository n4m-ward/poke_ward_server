...
if(NpcHandler == nil) then
	...
	function NpcHandler:releaseFocus(focus)
		...
		if ( isPlayer(focus) ) then --Even if it can not exist, we need to prevent it.
	...
	function NpcHandler:processModuleCallback(id, ...)
		...
		for i, module in pairs(self.modules) do
			...
			elseif (id == CALLBACK_ONRELEASEFOCUS and module.callbackOnReleaseFocus ~= nil) then
				tmpRet = module:callbackOnReleaseFocus(unpack(arg))
			end
	...
	function NpcHandler:unGreet(cid)
		...
		if(callback == nil or callback(cid)) then
			if(self:processModuleCallback(CALLBACK_FAREWELL)) then
				...
				local parseInfo = { [TAG_PLAYERNAME] = (cid and getCreatureName(cid) or -1) }				
				self:resetNpc(cid)
...
function NpcHandler:onBuy(cid, itemid, subType, amount, ignoreCap, inBackpacks)
		...
		it = {1990, 1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004, 12325, 12326, 12327, 12328, 12329, 12333}, --id dos itens
		total = 15 --total de itens que pode ser comprado por vez
		}

		count = 0
		for i = 1, #config.it do
			count = count + getPlayerItemCount(cid, config.it[i])
		end

		if count >= 16 then
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_ORANGE, "Você já possui 15 backpacks.")
			return false
		end
