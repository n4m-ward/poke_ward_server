...
sudo iptables -t filter -A INPUT -p tcp --dport 1026 -j ACCEPT
sudo iptables -t filter -A OUTPUT -p tcp --dport 1026 -j ACCEPT
  
# Active Localhost OPENVPN SECURITY
sudo iptables -t filter -A INPUT -p tcp --dport 22 -s  127.0.0.1 -j ACCEPT
sudo iptables -t filter -A INPUT -p tcp --dport 1026 -s  127.0.0.1 -j ACCEPT
