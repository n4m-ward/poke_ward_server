...
bool ProtocolLogin::parseFirstPacket(NetworkMessage& msg)
{
	...
	std::string newVersion = msg.GetString();
	...
	if(ConnectionManager::getInstance()->isDisabled(getIP(), protocolId))
	{
		...
		disconnectClient(0x0A, "Muitas tentativas de conexï¿½es feitas por este IP, tente novamente mais tarde.");
		return false;
	}
	...
	if (newVersion != g_config.getString(ConfigManager::GAMEVERSION))
	{
		getConnection()->close();
		return false;
	}
	...
	{
		disconnectClient(0x0A, "Servidor está em manutenção.");
		return false;
	}
