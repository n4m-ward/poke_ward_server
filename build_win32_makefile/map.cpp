	...
	std::list<Direction>& listDir, int32_t maxSearchDist /*= -1*/)
{
	...
	{
		if(!(n = nodes.getBestNode(destPos)))
		{
			listDir.clear();
...
bool Map::getPathMatching(const Creature* creature, const Position& targetPos, std::list<Direction>& dirList,
	const FrozenPathingConditionCall& pathCondition, const FindPathParams& fpp)
{
	...
	{
		if(!(n = nodes.getBestNode(targetPos)))
		{
