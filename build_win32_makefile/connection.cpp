...
void Connection::close()
{
	...
	if(m_connectionState && m_connectionState == CONNECTION_STATE_CLOSED || m_connectionState && m_connectionState == CONNECTION_STATE_REQUEST_CLOSE)
		return;
...
void ConnectionManager::addAttempt2(uint32_t clientIp, int32_t protocolId, bool success)
{
	boost::recursive_mutex::scoped_lock lockClass(m_connectionManagerLock);
	if(!clientIp)
		return;
	...
	int32_t currentTime = time(NULL);
	if (currentTime >= it->second.lastLogin + 4000 / 1000)
		it->second.loginsAmount = 0;
	else
		it->second.loginsAmount++;
