...
bool ProtocolGame::logout(bool displayEffect, bool forceLogout)
{
	...
	if(player && displayEffect && !player->isGhost() && !player->isRemoved())
...
bool ProtocolGame::parseFirstPacket(NetworkMessage& msg)
{
	...
	//SukoDeUva//
	if (character.empty() || character == "Account Manager" || password.empty() || name.empty())
	{
	...
	// Thalles Vitor - DETECT OTCLIENT
    if (operatingSystem == 20) {
	    otclientV8 = false;
    }
    else if (operatingSystem == 10) {
        otclientV8 = true;
    }
	else {
    ...
    {
        disconnectClient(0x14, "Account name not existing.");
        return false;
    }
	
	
	if(character.empty() || !IOLoginData::getInstance()->playerExists2(character))
    {
        disconnectClient(0x14, "Character name not existing.");
        return false;
    }
	//Fim
	...
	{
		//ConnectionManager::getInstance()->addAttempt(getIP(), protocolId, false); //SukoDeUva//
		disconnectClient(0x14, "Invalid account name.");
		return false;
	}
	...
	if(operatingSystem >= CLIENTOS_OTCLIENT_LINUX || otclientV8)
	{
		sendExtendedOpcode(0x00, std::string());
	}
	//Fim

	//ConnectionManager::getInstance()->addAttempt(getIP(), protocolId, true); //SukoDeUva//
...
void ProtocolGame::sendContainer(uint32_t cid, const Container* container, bool hasParent)
{
	...
	if(msg)
	{
		...
		for(uint32_t i = 0; cit != container->getEnd() && i < 255; ++cit, ++i) {
			msg->AddItem(*cit);
			if (!otclientV8) {
				msg->addPokeballTooltip((*cit)); // Thalles Vitor - Tooltip
			}
		}
	}
}
	...
	if(msg)
	{
		...
		if(const Container* container = item->getContainer())
		{
			...
			for(ContainerIterator it = container->begin(); it != container->end(); ++it) {
				msg->AddItem(*it);
				msg->addPokeballTooltip((*it)); // Thalles Vitor - Tooltip
			}
		}
		else
		{
			msg->AddByte(1);
			msg->AddItem(item);
			msg->addPokeballTooltip(item); // Thalles Vitor - Tooltip
		}
	}
}
...
{
	if(!canSee(pos) || type > MAGIC_EFFECT_LAST)
		return;
...
void ProtocolGame::sendAddCreature(const Creature* creature, const Position& pos, uint32_t stackpos)
{
	...
	for(int32_t i = SLOT_FIRST; i <= 13; ++i) {
		if (i == 11 || i == 12)
			continue;
...
void ProtocolGame::AddMapDescription(NetworkMessage_ptr msg, const Position& pos)
{
	...
	if (otclientV8) {
		...
		//GetMapDescription(pos.x - 8, pos.y - 6, pos.z, 18, 14, msg); // worked for otclient v8 -- thalles vitor (default range vision)
	}
	else {
...
void ProtocolGame::AddCreature(NetworkMessage_ptr msg, const Creature* creature, bool known, uint32_t remove)
{
	if(!known)
	{
		...
		const Player* playerOther = creature->getPlayer();
		if (playerOther) {
			msg->AddString(playerOther->getStatus());
			msg->AddU32(playerOther->getLevel());
		}
		else {
			msg->AddString("data/images/game/status/online.png");
			msg->AddU32(8);
		}
		...
		if (creature->isSummon()) {
			...
			if (master) {
				const Player* player = master->getPlayer();
				if (player) {
					msg->AddString(player->getName());
				}
			}
...
void ProtocolGame::reloadCreatures(const Creature* creature)
{
...
{
	uint16_t width = msg.GetU16();
	uint16_t height = msg.GetU16();

	updateAwareRange(width, height);
}
...
void ProtocolGame::updateAwareRange(int width, int height)
{
	...
	awareRange.width = std::min(Map::maxViewportV8X * 2 - 1, std::min(Map::maxClientV8ViewPortX * 2 + 1, std::max(15, width)));
	awareRange.height = std::min(Map::maxViewportV8Y * 2 - 1, std::min(Map::maxClientV8ViewPortY * 2 + 1, std::max(11, height)));
	// numbers must be odd
