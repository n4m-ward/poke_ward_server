...
class Item : virtual public Thing, public ItemAttributes
{
	public:
		...
		// Thalles Vitor
		uint16_t getOLDID() const {return oldId;}
		void setOLDID(uint16_t oldId) {oldId = oldId;}
		...
		virtual bool unserializeItemNode(PropStream& propStream) {
			if (propStream.size() > 0) {
				unserializeAttr(propStream);
				return true;
			}
...
inline std::string Item::getPokemon() const
{
