...
Creature::~Creature()
{
	...
	{
		(*cit)->setAttackedCreature(NULL);
		(*cit)->setGuardian(NULL);
		(*cit)->unRef();
	}

	summons.clear();
	guardians.clear(); // Thalles
	conditions.clear();
	eventsList.clear();
}
...
bool Creature::canSeeCreature(const Creature* creature) const
{
	...
	if (creature->isGhost() && !canSeeInvisibility())
		return false;
...
void Creature::onRemovedCreature()
{
	...
	if(guardianMaster && !guardianMaster->isRemoved()) {
		guardianMaster->removeGuardian(this);
	}
}
...
bool Creature::onDeath()
{
	...
	for(DeathList::iterator it = deathList.begin(); it != deathList.end(); ++it, ++i)
	{
		...
		if(i < size)
		{
			...
			else if (it->getKillerCreature()->getGuardian()) // Thalles Vitor - Guardian
				tmp = it->getKillerCreature()->getGuardian(); 
		}
...
void Creature::dropCorpse(DeathList deathList)
{
	...
	if (!deathEvents.empty() && deathEvents.size() > 0) {
...
void Creature::onGainExperience(double& gainExp, bool fromMonster, bool multiplied)
{
	...
	if(guardianMaster) // Thalles Vitor - Guardian
	{
		guardianMaster->onGainExperience(gainExp, fromMonster, multiplied);
	}

	else if(!multiplied)
		gainExp *= g_config.getDouble(ConfigManager::RATE_EXPERIENCE);

	int16_t color = TEXTCOLOR_GREEN;
...
void Creature::onGainSharedExperience(double& gainExp, bool fromMonster, bool multiplied)
{
	...
	if(guardianMaster) // Thalles Vitor - Guardian System
	{
		gainExp = gainExp / 6;
		guardianMaster->onGainSharedExperience(gainExp, fromMonster, multiplied);
	}
