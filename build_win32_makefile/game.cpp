...
void Game::start(ServiceManager* servicer)
{
		...
		&& g_config.getNumber(ConfigManager::GLOBALSAVE_H) <= 24)
	{
		...
		else if(theTime && theTime->tm_hour == prepareGlobalSaveHour)
		{
			if(theTime && theTime->tm_min >= 55 && theTime->tm_min <= 59)
			{
				if(theTime && theTime->tm_min >= 57)
					setGlobalSaveMessage(0);

				if(theTime && theTime->tm_min == 59)
					setGlobalSaveMessage(1);
...
// Thalles
void Game::saveGameStateAntiRollback()
{
	std::cout << "> Saving server..." << std::endl;
	uint64_t start = OTSYS_TIME();
	if(gameState == GAME_STATE_NORMAL)
	...
	uint32_t spriteId/* = 0*/, stackposType_t type/* = STACKPOS_NORMAL*/)
{
	...
	else if(pos.y & 0x40)
	{
		...
		if(Container* parentcontainer = player->getContainer(fromCid)) {
			if (parentcontainer && parentcontainer != NULL && !parentcontainer->isRemoved()) { // Thalles
				Item* parentItem = parentcontainer->getItem(slot);
				if (parentItem && !parentItem->isRemoved()) {
					return parentItem;
				}
			}
		}
	...
	Item* slotInv = player->getInventoryItem((slots_t)static_cast<uint8_t>(pos.y));
	if (slotInv && !slotInv->isRemoved()) {
		return slotInv;
	}
...
Creature* Game::getCreatureByName(std::string s)
{
	...
	{
		if(it->second && !it->second->isRemoved() && asLowerCaseString(it->second->getName()) == s)
			return it->second;
	}
	...
	const Position& movingCreaturePos, const Position& toPos)
{
	...
	// Thalles Vitor - Nao Empurrar Player pra TP
	Tile* checkHaveTp = map->getTile(movingCreaturePos);
	if (checkHaveTp) {
		Item* tps = checkHaveTp->getItem();
		if (tps && tps->getID() == 1387)
		{
			player->sendCancelMessage(RET_NOTPOSSIBLE);
			return false;
		}
...
ReturnValue Game::internalMoveCreature(Creature* creature, Direction direction, uint32_t flags/* = 0*/)
{
	...
	if (!toTile->ground) // Thalles
		return RET_NOTPOSSIBLE;

	// Thalles Vitor - Andar //
		CreatureEventList walkEvent = creature->getCreatureEvents(CREATURE_EVENT_WALK);
		if (!walkEvent.empty() && walkEvent.size() > 0) {
			for(CreatureEventList::iterator it = walkEvent.begin(); it != walkEvent.end(); ++it)
			{
	...
	// Thalles Vitor - Poke House
		if (Monster* monster = creature->getMonster()) {
			std::string value;
			if (monster->getStorage(999, value) && atoi(value.c_str()) >= 1) {
				Tile* tilee = map->getTile(destPos);
				if (!tilee) {
					return RET_CANNOTTHROW;
				}

				Tile* tilee2 = map->getTile(Position(destPos.x + 1, destPos.y, destPos.z));
				if (!tilee2) {
					return RET_CANNOTTHROW;
				}

				Tile* tilee3 = map->getTile(Position(destPos.x, destPos.y + 1, destPos.z));
				if (!tilee3) {
					return RET_CANNOTTHROW;
				}

				HouseTile* houseTile1 = tilee->getHouseTile();
				HouseTile* houseTile2 = tilee2->getHouseTile();
				HouseTile* houseTile3 = tilee3->getHouseTile();
				if (houseTile1 && houseTile2 && houseTile3) {
					internalCreatureTurn(monster, direction);
					internalTeleport(monster, destPos, true);
					return RET_NOERROR;
				}
	...
	ReturnValue ret = internalMoveCreature(NULL, creature, fromTile, toTile, flags);
	if(ret != RET_NOERROR)
	{
		ret = RET_NOTPOSSIBLE;
...
ReturnValue Game::internalMoveCreature(Creature* actor, Creature* creature, Cylinder* fromCylinder, Cylinder* toCylinder, uint32_t flags/* = 0*/)
{
	...
	if (creature && !creature->isRemoved()) {
		...
		if (fromCylinder) {
			Tile* tile = fromCylinder->getTile();
			if (tile && !tile->isRemoved()) {
				tile->moveCreature(actor, creature, toCylinder);
			}
		}
		...
		while((subCylinder = toCylinder->__queryDestination(tmp, creature, &toItem, flags)) != toCylinder)
		{
			...
			if (tile && !tile->isRemoved() && subCylinder) {
				tile->moveCreature(actor, creature, subCylinder);
			}
	...
	uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint8_t count)
{
	...
	if (player->hasFlag(PlayerFlag_CannotMoveItems))
		return false;
	...
	Tile* fromMapTile = fromCylinder->getTile();
	if (!fromMapTile || !fromMapTile->ground || fromMapTile->isRemoved())
	{
		return false;
	}

	if (toCylinder->isRemoved()) { // Thalles
		return false;
	}

	Tile* toMapTile = toCylinder->getTile();
	if (!toMapTile)
	{
		return false;
	}

	if (toMapTile->isRemoved())
	{
		return false;
	}
	...
	const Position& mapFromPos = fromMapTile->getPosition();
	const Position& mapToPos = toMapTile->getPosition();
	...
	if(item && item->isHangable() && toMapTile && toMapTile->hasProperty(SUPPORTHANGABLE))
	{
		//destination supports hangable objects so need to move there first
		if(toMapTile->hasProperty(ISVERTICAL))
		{
		...
		else if(toMapTile->hasProperty(ISHORIZONTAL))
		{
		...
		if(!Position::areInRange<1,1,0>(playerPos, mapToPos) && !player->hasCustomFlag(PlayerCustomFlag_CanMoveFromFar))
		{
			...
			if(toMapTile->hasProperty(ISHORIZONTAL))
				walkPos.y -= -1;
	...
	// Thalles Vitor - Mover Item para House //
		Tile* checkTile = map->getTile(toPos);
		if (checkTile && checkTile->ground) {
			HouseTile* houseTile = checkTile->getHouseTile();
			if (houseTile) {
				House* house = houseTile->getHouse();

				if (house && house->isInvited(player) && player->getGUID() != house->getOwner()) {
					player->sendCancel("Apenas o dono pode jogar items dentro da casa.");
					return false;
				}
			}
		}
	...
	if (!moveitemEvents.empty() && moveitemEvents.size() > 0) {
...
// Thalles Vitor
ReturnValue Game::internalAddLoot(Cylinder* fromCylinder, Player* player, Item* item)
{
	if (!fromCylinder)
		return RET_NOTPOSSIBLE;
	...
	Item* bp = player->getInventoryItem((slots_t)3);
	if (!bp) 
		return RET_NOTPOSSIBLE;

	Container* bpC = bp->getContainer();
	if (!bpC)
		return RET_NOTPOSSIBLE;

	uint32_t backpackLotada = 0;
	for(ContainerIterator it = bpC->begin(); it != bpC->end(); it++)
	{
		Item* item2 = (*it);
		if (item2) {	
			++backpackLotada;
	...
	Item* coins = player->getInventoryItem((slots_t)10);
	if (!coins)
		return RET_NOTPOSSIBLE;
	...
	uint32_t coinsLotado = 0;
	for(ContainerIterator it = coinsC->begin(); it != coinsC->end(); it++)
	{
		Item* item2 = (*it);
		if (item2) {	
			++coinsLotado;
	...
	if (backpackLotada >= 32 && coinsLotado >= 8) {
		ReturnValue ret = internalPlayerAddItem(NULL, player, item, true);
		if(ret != RET_NOERROR)
		{
			return RET_NOTPOSSIBLE;
		}
	...
	ReturnValue ret2 = internalMoveItem(NULL, fromCylinder, player, INDEX_WHEREEVER, item, item->getItemCount(), NULL, FLAG_NOLIMIT);
	if(ret2 != RET_NOERROR)
	{
		return RET_NOTPOSSIBLE;
	}
	...
	int32_t index, Item* item, uint32_t count, Item** _moveItem, uint32_t flags /*= 0*/)
{
	...
	if (fromCylinder->isRemoved())
		return RET_NOTPOSSIBLE;

	if(!toCylinder)
		return RET_NOTPOSSIBLE;

	if (toCylinder->isRemoved())
		return RET_NOTPOSSIBLE;
	...
	/* if (toCylinder) {
		Item* catchBag = toCylinder->getItem();
		if (catchBag && catchBag->getID() == 28146 && !item->getPokemon().empty()) {
			ret = RET_NOERROR;
		}
	} */
...
ReturnValue Game::internalRemoveItem(Creature* actor, Item* item, int32_t count /*= -1*/, bool test /*= false*/, uint32_t flags /*= 0*/)
{
	...
	if (container) {
		uint32_t ownerId = container->getCorpseOwner();
		if (ownerId > 0) {
			Player* owner = getPlayerByID(ownerId);
			if (owner && !owner->isRemoved()) {
				owner->autoCloseContainers(container);
			}
		}
	}
...
void Game::addMoney(Cylinder* cylinder, int32_t money, uint32_t flags /*= 0*/)
{
	...
	Cylinder* c = cylinder;
	/* Creature* creature = c->getCreature();
	if (creature) {
		Player* player = creature->getPlayer();

		if (player) {
			Item* bp = player->getInventoryItem((slots_t)SLOT_AMMO);
			if (bp) {
	...
	for(IntegerMap::reverse_iterator it = moneyMap.rbegin(); it != moneyMap.rend(); ++it)
	{
		...
		if(tmp != 0)
		{
			do
			{
				...
				if(internalAddItem(NULL, c, remaindItem, INDEX_WHEREEVER, FLAG_NOLIMIT) != RET_NOERROR)
...
ReturnValue Game::internalTeleport(Thing* thing, const Position& newPos, bool pushMove, uint32_t flags /*= 0*/)
{
	...
	if (toTile && !toTile->isRemoved())
	{
		Creature* creature = thing->getCreature();
		if (creature && !creature->isRemoved())
		{
			///ReturnValue ret = toTile->__queryAdd(INDEX_WHEREEVER, thing, 1, NULL);
			//if (ret == RET_NOERROR) {
				if (toTile) {
					Tile* playerTile = creature->getTile();
					if (!playerTile)
						return RET_NOTPOSSIBLE;

					if(Position::areInRange<1,1,0>(creature->getPosition(), newPos) && pushMove)
						playerTile->moveCreature(NULL, creature, toTile, false);
					else
						playerTile->moveCreature(NULL, creature, toTile, true);
				}
...
bool Game::playerOpenPrivateChannel(uint32_t playerId, std::string& receiver)
{
	Player* player = getPlayerByID(playerId);
	if(!player) {
		player->sendCancel("A player with this name does not exist or is offline.");
		return false;
	}
	...
	uint32_t tradePlayerId, uint16_t spriteId)
{
	...
	// Thalles Vitor
	uint32_t totalCatchBag = 0;
	uint32_t totalCatchBag2 = 0;

	Item* catchBag1 = player->getInventoryItem((slots_t)13);
	if (catchBag1) {
		Container* container1 = catchBag1->getContainer();
		if (container1) {
			for (ContainerIterator it = container1->begin(); it != container1->end(); ++it) {
				Item* mainItem = (*it);
				if (mainItem)
				{
					++totalCatchBag;
	...
	Item* catchBag2 = tradePartner->getInventoryItem((slots_t)13);
	if (catchBag2) {
		Container* container2 = catchBag2->getContainer();
		if (container2) {
			for (ContainerIterator it = container2->begin(); it != container2->end(); ++it) {
				Item* mainItem = (*it);
				if (mainItem)
				{
					++totalCatchBag2;
	...
	if (totalCatchBag > 0 || totalCatchBag2 > 0)
	{
		player->sendTextMessage(MSG_INFO_DESCR, "Algum dos jogadores possui item na catch bag.");
		tradePartner->sendTextMessage(MSG_INFO_DESCR, "Algum dos jogadores possui item na catch bag.");
		return false;
	}
...
bool Game::playerAcceptTrade(uint32_t playerId)
{
	...
	if(ret1 == RET_NOERROR && ret2 == RET_NOERROR)
	{
		...
		if(ret1 == RET_NOERROR && ret2 == RET_NOERROR)
		{
			...
			internalMoveItem(player, cylinder1, cylinder2, INDEX_WHEREEVER, tradeItem1, tradeItem1->getItemCount(), NULL);
			internalMoveItem(tradePartner, cylinder2, cylinder1, INDEX_WHEREEVER, tradeItem2, tradeItem2->getItemCount(), NULL);
			...
			// Thalles Vitor
			Item* slot1 = player->getInventoryItem((slots_t)8);
			if (!slot1) {
				Item* slot2 = player->getInventoryItem((slots_t)7);
				if (slot2 && slot2->getID() != 2395) {
					transformItem(slot2, 2395);
				}
			}

			Item* slot3 = tradePartner->getInventoryItem((slots_t)8);
			if (!slot3) {
				Item* slot4 = tradePartner->getInventoryItem((slots_t)7);
				if (slot4 && slot4->getID() != 2395) {
					transformItem(slot4, 2395);
				}
			}
...
bool Game::playerLookInTrade(uint32_t playerId, bool lookAtCounterOffer, int32_t index)
{
	...
	{
		if(!(*it)->executeLook(player, tradeItem, tradeItem->getPosition(), 0, lookDistance))
			deny = true;
	}
...
bool Game::internalCloseTrade(Player* player)
{
	...
	if (player->getTradeState() == TRADE_NONE || player->getTradeState() == TRADE_NONE)
		return false;
	...
	if (player && tradePartner) {
		...
		player->sendTradeClose();
		if(tradePartner && tradePartner != NULL)
		{
			if(tradePartner->getTradeItem())
			{
				...
				if (tradePartner && tradePartner->tradeItem && tradePartner->tradeItem != NULL) {
...
bool Game::playerLookAt(uint32_t playerId, const Position& pos, uint16_t spriteId, int16_t stackpos)
{
	...
	bool deny = false;
	CreatureEventList lookEvents = player->getCreatureEvents(CREATURE_EVENT_LOOK);
	if (!lookEvents.empty() && lookEvents.size() > 0) {
		for(CreatureEventList::iterator it = lookEvents.begin(); it != lookEvents.end(); ++it)
		{
...
bool Game::playerYell(Player* player, const std::string& text)
{
	...
	{
		if(Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_MUTED, 2000, 0, false, 1))
...
{
	return map->getPathMatching(creature, targetPos, dirList, FrozenPathingConditionCall(targetPos), fpp);
}
...
{
	Creature* creature = getCreatureByID(creatureId);
	if(creature && !creature->isRemoved() && creature->getHealth() > 0)
	{
		creature->onWalk();
		cleanup();
	}
	...
	MagicEffect_t hitEffect/* = MAGIC_EFFECT_UNKNOWN*/, TextColor_t hitColor/* = TEXTCOLOR_UNKNOWN*/, bool force/* = false*/)
{
	...
	if(healthChange > 0)
	{
		...
		if (!statsChangeEvents.empty() && statsChangeEvents.size() > 0) {
	...
	else
	{
		...
		// Thalles Vitor - Summon
		if (attacker && target && attacker->getMonster() && target->getPlayer() && target->summons.size() > 0)
			return false;
		...
		if(damage != 0)
		{
			...
			if(damage > 0)
			{
				...
				if(textColor != TEXTCOLOR_UNKNOWN && magicEffect != MAGIC_EFFECT_UNKNOWN)
				{
					char buffer[20];
