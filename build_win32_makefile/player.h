...
class Player : public Creature, public Cylinder
{
	public:
		...
		void reloadCreatures(const Creature* creature)
            {if(client) client->reloadCreature(creature);}
		...
		bool selectPokeball(uint16_t pokeballsNumber);
	...
	protected:
		...
		bool hasCapacity(const Item* item, uint32_t count, uint32_t index) const;

		// Thalles Vitor
		std::string getStatus() const;
		void setStatus(std::string statuss) { status = statuss; }
